...
class ColeColeModulusModel(BaseModel):
    def __init__(self):
        super().__init__("ColeCole Modulus")
    ...
    def model_function(self, f, M_inf, M_s, tau, alpha):
        w = 2 * np.pi * f
    ...
    def fit(self, f, M_real, M_imag, user_params=None):
        def model_real(f, M_inf, M_s, tau, alpha):
            return np.real(self.model_function(f, M_inf, M_s, tau, alpha))

        def model_imag(f, M_inf, M_s, tau, alpha):
            return np.imag(self.model_function(f, M_inf, M_s, tau, alpha))
